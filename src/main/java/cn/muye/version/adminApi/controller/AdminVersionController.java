package cn.muye.version.adminApi.controller;
...
public class AdminVersionController {
    ...
    public AjaxResult adminPostVersion(@ApiParam(value = "版本对象") @RequestBody Version version) {
        if (version != null && (version.getVersionCode() == null || version.getUrl() == null)) {
            return AjaxResult.failed(AjaxResult.CODE_PARAM_MISTAKE_FAILED, "信息不全，请补全后提交");
        }
        Version versionDb = adminVersionService.getByCode(version.getVersionCode());
        if (versionDb != null && version.getVersionCode().toLowerCase().equals(versionDb.getVersionCode().toLowerCase()) && !versionDb.getId().equals(version.getId())) {
            return AjaxResult.failed(AjaxResult.CODE_ERROR_FAILED, "版本号重复");
        }
